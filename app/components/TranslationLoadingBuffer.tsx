 ...
 * Only shows loading state when content is actively being translated to Hindi
 */
export function TranslationLoadingBuffer({
  children,
  fallback,
}: TranslationLoadingBufferProps) {
  let contentLoadingCount = 0;
  let language: "en" | "hi" = "en";

  try {
    const context = useTranslation();
    contentLoadingCount = context.contentLoadingCount;
  ...
  // Only show loading state when there's actual content loading in Hindi
  if (contentLoadingCount > 0 && language === "hi") {
    return (
      fallback || (
 ...
 * Shows only when content is actively being translated
 */
export function TranslationLoadingIndicator() {
  ...
  // Only show when there's actual content loading in Hindi
  if (contentLoadingCount === 0 || language === "en") {
    return null;
  }
